...
2. Open `PLAN BUILDER.html` directly in a modern browser (Chrome/Edge/Firefox/Safari). You can double-click the file or drag it into a browser window. A stamped versioned copy is also available as `PLAN BUILDER v2025.1.5.html` if you want to keep multiple builds side by side.
...
- A new **Image Buttons** tab lets you drop in PNG/JPG assets and wire them to HTML embeds, galleries, or links with adjustable size, opacity, rotation, and rounded corners.
